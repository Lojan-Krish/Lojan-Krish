...
I build the whole thing: design, front end, back end, performance, technical SEO, DNS and mail. One person, no handoff.
...
### BBK Sri Lanka — employee attendance & shift platform

`Python` `FastAPI` `PostgreSQL` `React` `Azure` `Docker` — *in development*
...
`C# 7.3` `.NET Framework 4.8` `Windows Forms` `SQL Server` `ADO.NET` — *my own project, built solo*
...
### Support Our Schools — Sri Lanka Map · WordPress plugin

`PHP` `MySQL` `REST API` `PWA` `i18n`
...
## Live sites I built from scratch and still maintain
...
90+ Google PageSpeed on every one. Load times down from over 8 seconds to under 2.
...
| | |
|---|---|
| **15+** | production WordPress sites shipped since 2023, built from scratch, still maintained |
| **90+** | Google PageSpeed on 100% of deployments |
| **11** | domains under management — DNS, SSL/TLS, aliases and mail |
| **150%** | organic search visibility lift for a client |
| **60% / 20%** | faster page loads and lower bounce on the 360 Accountants rebuild |
...
**Software Engineer · Web Developer · Graphic Designer** — Jaffna, Sri Lanka
Open to roles in the UK and Europe, remote freelance work worldwide.

I build the whole thing: design, front end, back end, performance, technical SEO,
DNS and mail. One person, no handoff.
...
### BBK Sri Lanka — employee attendance & shift platform · *in development*
`Python` `FastAPI` `PostgreSQL` `React` `Azure` `Docker`

Internal platform for 200 staff across three branches on six shift patterns.
76 FastAPI routes over 18 PostgreSQL models behind a service and repository layer;
35 React pages; trilingual interface in English, Sinhala and Tamil.
Microsoft 365 SSO through Azure AD with OTP second factor, Microsoft Teams and
TSheets integration, Celery for scheduled work, WebSocket for live state.
AES-256 at rest, role-based access control with an access matrix, audit log on every action.

### SmartMed Pharmacy Management System — *my own project*
`C# 7.3` `.NET Framework 4.8` `Windows Forms` `SQL Server` `ADO.NET`

Desktop system, 446,000 lines of hand-written C# across 871 files and 1,362 classes —
only 2 designer-generated files, so the entire Windows Forms UI is written by hand.
Four layers, repository pattern over ADO.NET, owner-drawn custom control library.
PBKDF2-SHA256 hashing, TOTP two-factor with QR enrolment, RBAC, hash-chained audit log.
Debounced multi-field search with a Levenshtein fallback; complexity documented.
...
24,325 lines of PHP across 65 files, 18 module classes, 8 custom MySQL tables,
28 shortcodes, 30 AJAX endpoints and 7 REST API routes. Custom editor role whose
capability model lets the content team manage schools, events and campaigns but never
settings or secrets — with a `user_has_cap` filter guaranteeing an administrator can
never be locked out. PWA service worker and manifest, Gutenberg blocks, XLSX export,
scheduled digests, backup, cross-site sync. Full Tamil localisation (ta_LK, ta_IN).
...
- **11** domains under management — DNS, SSL/TLS, aliases and mail
- **150%** organic search visibility lift for a client
- **60%** faster page loads and **20%** lower bounce on the 360 Accountants rebuild
